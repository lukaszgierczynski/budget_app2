...
function UserHomeScreen() {
  ...
  const specificPeriod = 90;

  let isFormValid;
  isFormValid = isStartDateBeforeEndDate && isGapLessThanPeriod;

  const expenesesSumInPeriod = graphData.filter(
    (transacationsList) => transacationsList.name === "wydatki"
  )[0]
    ? graphData
        .filter((transacationsList) => transacationsList.name === "wydatki")[0]
        .y.reduce((acc, wydatki) => {
          return acc + wydatki;
        })
    : 0;
  const incomesSumInPeriod = graphData.filter(
    (transacationsList) => transacationsList.name === "dochody"
  )[0]
    ? graphData
        .filter((transacationsList) => transacationsList.name === "dochody")[0]
        .y.reduce((acc, dochody) => {
          return acc + dochody;
        })
    : 0;
  ...
  useEffect(() => {
    if (!areTransactions) {
      ...
      setGraphData(
        transactionsAggrPerDay(transactions, startDate, endDate).map(
          (data) => ({
            ...data,
            type: "bar",
          })
      ...
      setGraphTitle(`Wydatki i dochody w okresie od ${startDate} do ${endDate}`);
      setSumTitle(`W okresie od ${startDate} do ${endDate}:`);
    }
  }, [areTransactions]);
  ...
  return (
    <Container>
      ...
      (<><div className="d-flex flex-column justify-content-center aling-items-center mt-3">
        ...
        <p className="text-center" style={{ marginBottom: "5px" }}>
          suma wydatków wyniosła {`${expenesesSumInPeriod}`}
        </p>
        <p className="text-center" style={{ margin: "0" }}>
          suma dochodów wyniosła {`${incomesSumInPeriod}`}
        </p>
      </div>
